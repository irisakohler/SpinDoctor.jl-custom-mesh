"""
    couple_flux_matrix_parallel(model, Q_blocks[, symmetrical])

Create global flux matrix with coupling between compartments.
"""
function couple_flux_matrix_parallel(model, Q_blocks, symmetrical=false)
    (; mesh, ρ, κ) = model
    ...
    Q_threads = [spzeros(npoint, npoint) for _ in 1:Threads.nthreads()]

    Threads.@threads for iboundary in 1:nboundary
        Qloc = Q_threads[threadid()]
        ...
        if ntouch == 1
            cmpt = cmpts_touch[1]
            inds = get_inds(cmpt)
            κ[iboundary] ≈ 0 || (Qloc[inds, inds] += κ[iboundary] * Q_blocks[cmpt, iboundary])
